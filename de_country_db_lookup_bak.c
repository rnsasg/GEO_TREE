...
void build_ds_by_read_csv(const char *filename)
{
...
de_tree_node_t *read_csv_and_build_ds(de_tree_node_t *root,char *filename)
{
 	FILE *fptr=NULL;
        char c;
        unsigned int start_ip=0,end_ip=0;
	...
	de_data_row_t temp;
        ...
        while(fscanf(fptr,"%d;%d;%s\n",&start_ip,&end_ip,country_name)!=EOF)
        {
                country_name[3]='\0';
               // printf("%ld %ld %s\n",start_ip,end_ip,country_name);
		temp.start_ip=start_ip;
		temp.end_ip=end_ip;
		strncpy(temp.three_letter_cn_name,country_name,4);
		printf("\n In %s : Start_IP: %d \t End_IP: %d \t Country Name: %s \n",__FUNCTION__,temp.start_ip,temp.end_ip,temp.three_letter_cn_name);
	...
	int i=0;
		...
		printf(" 5. Tree Height 	  :\n");
			...
			case 1: printf("--------- Insert using CSV file ---------------\n");
                                ...
                                scanf("%s",filename);
				t = clock();
                                root=read_csv_and_build_ds(root,filename);
				t = clock() - t;
                                time_taken = ((double)t)/CLOCKS_PER_SEC;
                                printf("read_csv_and_build_ds() took %f seconds to execute \n", time_taken);
                                break;
